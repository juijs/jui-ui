...
jui.defineUI("chart.builder", ["jquery", "util.base", "util.svg", "util.color"], function($, _, SVGUtil, ColorUtil) {
  ...
  var UI = function() {
    ...
    function drawBrush(self, type) {
      ...
      if (draws != null) {
        for (var i = 0; i < draws.length; i++) {
          ...
          var Obj = jui.include("chart." + type + "." + draws[i].type), drawObject = (type == "widget") ? self.brush(i) : draws[i];
          ...
          if (type == 'widget') {
            _widget_object_list[i] = new Obj(self, draws[i]);
            drawBrushAfter(self, type, _widget_object_list[i].render());
          } else {
     ...
     *
     */
    function drawWidgetGrid(self) {
      var draws = _widget_object_list;

      if (draws != null) {
        for (var i = 0; i < draws.length; i++) {
          delete draws[i].widget.x;
          delete draws[i].widget.y;
          delete draws[i].widget.c;

          var drawObject = self.brush(i);

          if (_scales.x || _scales.x1) {
            if (!_.typeCheck("function", draws[i].widget.x)) {
              draws[i].widget.x = ( typeof drawObject.x1 !== 'undefined') ? _scales.x1[drawObject.x1 || 0] : _scales.x[drawObject.x || 0];
            }
          }
          if (_scales.y || _scales.y1) {
            if (!_.typeCheck("function", draws[i].widget.y)) {
              draws[i].widget.y = ( typeof drawObject.y1 !== 'undefined') ? _scales.y1[drawObject.y1 || 0] : _scales.y[drawObject.y || 0];
            }
          }
          if (_scales.c) {
            if (!_.typeCheck("function", draws[i].widget.c)) {
              draws[i].widget.c = _scales.c[drawObject.c || 0];
            }
          }
        }
      }
    ...
    this.page = function(pNo) {
      ...
      var dataList = this.options.data, limit = this.options.bufferCount, maxPage = Math.ceil(dataList.length / limit);
    ...
    this.next = function() {
      var dataList = this.options.data, limit = this.options.bufferCount, step = this.options.shiftCount;
    ...
    this.setCsv = function(csv) {
      var chartFields = [], csvFields = this.options.csv, csvNumber = this.options.csvNumber;
