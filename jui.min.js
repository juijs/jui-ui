...
},this.getLegendIcon=function(a,b){var c=[],d=b.target;b.legend&&(d=a.data());for(var e=d.length,f=0;e>f;f++){if(b.legend)var g=a.series(b.legend).text||d[f][b.legend];else var h=b.target[f],g=a.series(h).text||h;var i=a.svg.getTextRect(g),j=Math.min(i.width,i.height),k=j,l=a.svg.group({"class":"legend icon"});l.append(a.svg.rect({x:0,y:0,width:j,height:k,fill:a.color(f,b.colors)})),l.append(a.text({x:j+4,y:10,"text-anchor":"start"},g)),c.push({icon:l,width:j+4+i.width+10,height:k+4})}return c},this.draw=function(a){if(b){for(var h=a.svg.group({"class":"widget legend"}),i=0,j=0,k=0,l=0,m=0,n=0,o=0;o<b.brush.length;o++)for(var p=b.brush[o],q=a.brush(p),r=this.getLegendIcon(a,q),s=0;s<r.length;s++)h.append(r[s].icon),r[s].icon.translate(i,j),e||d?(i+=r[s].width,k+=r[s].width,n<r[s].height&&(n=r[s].height)):(f||g)&&(j+=r[s].height,l+=r[s].height,m<r[s].width&&(m=r[s].width));if(e||d){var j=e?a.y2()+a.padding("bottom")-n:a.y()-a.padding("top");"start"==c?i=a.x():"middle"==c?i=a.x()+(a.width()/2-k/2):"end"==c&&(i=a.x2()-k)}else if(f||g){var i=f?a.x()-a.padding("left"):a.x2()+a.padding("right")-m;"start"==c?j=a.y():"middle"==c?j=a.y()+(a.height()/2-l/2):"end"==c&&(j=a.y2()-l)}h.translate(Math.floor(i),Math.floor(j))}}};return a},"chart.draw"),jui.define("chart.widget.scroll",["util.base"],function(){var a=function(){function a(a,d){var g=!1,h=0,i=0;d.on("mousedown",function(a){g||(g=!0,h=a.offsetX,i=c)}),$("body").on("mousemove",function(j){if(g){var k=i+j.offsetX-h;0>k?k=0:k+b>a.width()&&(k=a.width()-b),d.translate(k,1),c=k;var l=Math.ceil((k+b/2)/totalCount)-1,m=0,n=0,o=Math.ceil(e/2);m=l>o?l-o:0,n=m+e-1,n+o>f&&(m=f-e,n=f-1),console.log(k,o,totalCount,e,f,m,n),a.zoom(m,n)}}),$("body").on("mouseup",function(){g&&(g=!1,h=0,i=0)})}var b=0,c=0,d=0,e=0,f=0;this.drawBefore=function(a){var c=a.options,g=(c.data.length-c.bufferCount)*c.shiftCount;f=c.data.length,e=c.bufferCount,b=a.width()/e,totalCount=a.width()/c.data.length,d=(a.width()-b)/g},this.draw=function(d){d.svg.group({autoRender:!1},function(){d.svg.rect({width:d.width(),height:7,fill:"#dcdcdc"});var e=d.svg.rect({width:b,height:5,fill:"#b2b2b2",stroke:"#9f9fa4","stroke-width":1}).translate(c,1);a(d,e)}).translate(d.x(),d.y2())}};return a},"chart.draw");